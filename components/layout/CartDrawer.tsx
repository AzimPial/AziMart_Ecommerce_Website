...
export function CartDrawer() {
  const { items: cart, addItem, removeItem, updateQty, clearCart, getTotal, getItemCount } = useCartStore();
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {cart.length === 0 ? (
            ...
            <ul className="space-y-4">
              {cart.map((item) => (
                <li key={item.product._id} className="flex gap-4 p-3 bg-muted/50 rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <p className="font-medium">{formatPrice(item.product.salePrice || item.product.price * item.qty)}</p>
                    </div>
                  </div>
                  <Button
                    variant="ghost"
